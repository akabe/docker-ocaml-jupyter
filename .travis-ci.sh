...
function build_image() {
    ...
    docker build -t akabe/ocaml-jupyter:$TAG dockerfiles/$TAG
    docker history akabe/ocaml-jupyter:$TAG
}
...
function test_image() {
    ...
    docker run -i --rm \
           -v "$PWD:$PWD" -w "$PWD" \
           -e LWT_LOG='* -> debug' \
           "akabe/ocaml-jupyter:$TAG" \
           jupyter nbconvert --to notebook --execute tests.ipynb
}
...
function deploy_image() {
    ...
    docker push akabe/ocaml-jupyter:$TAG

    for t in $ALIAS; do
        yellow_echo "[Deploy] Deploy image tag $TAG as $t"
        docker tag akabe/ocaml-jupyter:$TAG akabe/ocaml-jupyter:$t
        docker push akabe/ocaml-jupyter:$t
    done
}
